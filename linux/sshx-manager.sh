#!/usr/bin/env bash
# File: sshx-manager.sh (Linux)
# Version: 6.0.0
# Purpose: Comprehensive SSHX Management System for Linux
...
install_sshx() {
    log_status "INFO" "Starting installation..."
    
    # Dependencies
    log_status "INFO" "Checking dependencies..."
    for cmd in curl tar; do
        if ! command -v $cmd &> /dev/null; then
            log_status "WARNING" "Installing $cmd..."
    ...
    log_status "SUCCESS" "[OK] SSHX installed successfully"
    sleep 2
}
...
start_sshx() {
    ...
    # Run sshx and capture output to a temp file to find the URL
    local logfile="/tmp/sshx.log"
    nohup sshx > "$logfile" 2>&1 &
    
    log_status "INFO" "Waiting for URL..."
    sleep 5
    if grep -o 'https://sshx.io/s/[a-zA-Z0-9#]\+' "$logfile"; then
        local url=$(grep -o 'https://sshx.io/s/[a-zA-Z0-9#]\+' "$logfile")
...
uninstall_sshx() {
    log_status "WARNING" "(!) Uninstalling SSHX..."
    stop_sshx
    rm -f "$BIN_PATH"
    rm -rf "$STATE_DIR"
