#!/bin/zsh
# File: sshx-manager.zsh (macOS)
# Version: 6.0.0
# Purpose: Comprehensive SSHX Management System for macOS
...
check_sudo() {
    if [[ $EUID -ne 0 ]]; then
        log_status "ERROR" "This script must be run with sudo"
        exit 1
    fi
}
...
start_sshx() {
    ...
    # Ensure background process survives
    sshx > "$logfile" 2>&1 &!
